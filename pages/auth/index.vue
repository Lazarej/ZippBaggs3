      ...
      <form class="form form-create" v-else action="">
       ...
       <div class="column-form">
         ...
         <div class="input-cont">
          ...
          <small
            v-if="validation.error && getError('address') !== undefined"
            >{{ getError("address").message }}</small
          >
        </div>
       </div>
      </form>

        <button v-if="login === true" class="btn" @click="send">Connectez Vous</button>
        <button v-else class="btn" @click="send">Inscrivez vous</button>
 
    </div>
    <div v-if="login === true" class="changer">
        <h4>Crée un compte</h4>
        <p>Si vous n'avez pas encore de compte vous devez vous inscrire pour avec accès a nos produits </p>  
        <button class="btn" @click="login = false">Crée un compte</button>
    </div>
    <div v-else class="changer">
        <h4>Se connecter</h4>
        <p>Si vous avez deja un compte connecter vous pour pouvoir achetez nos produits </p>  
